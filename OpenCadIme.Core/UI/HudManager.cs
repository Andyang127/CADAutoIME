...
namespace OpenCadIme
{
    ...
    internal class HudManager : IDisposable
    {
        ...
        // ⭐ v0.3.1 优化：定时器降频，从 30ms 改为 100ms，降低 CPU 占用
        private const int HUD_TIMER_INTERVAL_MS = 100;
        // ⭐ v0.3.1 修复：注册表路径统一移至 AppConstants，与命名空间保持一致
        ...
        public void ShowWelcomeMessage(Document doc, string appVersion)
        {
            ...
            try
            {
                // ⭐ v0.3.1 修复：使用统一的注册表路径常量
                using (Microsoft.Win32.RegistryKey key = Microsoft.Win32.Registry.CurrentUser.CreateSubKey(AppConstants.RegistryPath))
                {
